...
\paragraph{Honest bound, not yet re-measured.} This fix responds to a defect discovered by analyzing the v12.0 run's own log, not to a second full execution; the next Kaggle run is the first empirical measurement of v12.1.
We can, however, bound its effect from the same log: on the 21 affected problems, always keeping the primary (the v12.0 behavior) scores 7/21, and an oracle that always picks whichever of the two is actually correct scores 14/21---raising the overall total from 124/150 (82.7\%) to at most 131/150 (87.3\%), i.e.\ parity with PAL, if the constrained Judge resolves these ties as well as an oracle; the floor is the unchanged 82.7\% if the Judge does no better than the coin flip it replaces. Because the Judge sees the problem text, the candidate equations, and executed code rather than resolving structurally, we expect a result between these bounds, closer to the upper one.
...
The v12.0 validation run (\Cref{sec:v12_validation}) confirms the signal is trustworthy once acted upon: SIV-verified answers were \textbf{100\% correct (12/12)} against 65.9\% for unverified answers.
The dedicated empirical evaluation---execution-audit catch rate, SHT-trigger reduction, Critic correction quality, the execution-vs-translation error decomposition, and the accuracy/cost delta against MAS-NoSIV (the ablation run)---remains the object of Experiment~3 (\Cref{sec:experiments}), with the analysis pipeline already in place (\Cref{app:siv_eval}); the v12.0/v12.1 numbers reported here establish that the signal is directionally sound, while the full ablation-controlled measurement is the immediate next deliverable.
...
Analyzing that run's own log surfaced a second, narrower selection-layer defect---a lone-primary-vs-lone-baseline tie was being resolved by a scoring artifact rather than evidence---which v12.1 fixes by escalating genuinely uncorroborated ties to the constrained judge instead of resolving them structurally (\Cref{sec:v12_validation}); a bounded estimate from the same log places the next run between 82.7\% (no worse) and 87.3\% (parity with PAL, if the judge resolves these ties as well as an oracle).
...
Future work should prioritize: (1)~running the MAS-NoSIV ablation on the same problem set to establish SIV's causal (not merely correlational) contribution, paired with a McNemar significance test (infrastructure already in place in \texttt{siv\_evaluation.py}); (2)~raising genuine (2+ given) blueprint coverage for SIV's evaluation, which stood at only 19.3\% (29/150) in our local open-weight run and directly limited how often Layer~2 could be exercised at all---we found gsm-hard yields roughly 2.6$\times$ more genuine blueprints than SVAMP (26\% vs.\ 10\%) and are reweighting the sampling mix accordingly, as a lower-cost complement to (4) below; (3)~combining SIV with an NL-layer verifier (e.g., FOBAR-style) to cover both error layers; (4)~systematic evaluation of heterogeneous agent configurations, particularly dedicating a stronger instruction-following model to the Mathematician role specifically, which our evidence suggests is the highest-leverage lever on blueprint yield; and (5)~larger-scale, multi-seed evaluation to establish statistical significance of the per-benchmark results and of Layer~2's genuine-case detector metrics, whose $n$ is currently too small (1 genuine SIV-verified case) to support a strong claim on its own.
