    ...
    def load_random_problems(self, datasets_list: List[str], num_problems: int, hardener: Optional[str] = None) -> List[Dict[str, str]]:
        ...
        for ds_name in datasets_list:
            ...
            try:
                ...
                # ----------------------------------------------------------------
                # [NEW 2026] GSM8K-Platinum — GSM8K's test set with its label
                # noise removed by expert re-annotation (madrylab, 2025).
                # Keys: "gsm8k-platinum", "gsm8k_platinum", "platinum"
                # HuggingFace: madrylab/gsm8k-platinum ("main", split="test")
                # Fields: question, answer ("#### N" tail), cleaning_status
                #
                # WHY THIS ONE MATTERS HERE. Of GSM8K's 1319 test problems, 110
                # were dropped as ambiguous or unanswerable and 10 were
                # relabelled, leaving 1209 (consensus 1100 / verified 99 /
                # revised 10). gsm-hard is BUILT from that same 1319-problem
                # test set, so every defect Platinum removed is still sitting in
                # gsm-hard underneath the numeric perturbation. This dataset is
                # therefore both a clean evaluation set AND an independent,
                # human-curated reference against which this project's own
                # mechanical gsm-hard audit can be cross-checked: match on the
                # question text, and a row Platinum threw out is a row our audit
                # should be suspicious of too.
                # ----------------------------------------------------------------
                elif ds_name_norm in ["gsm8k-platinum", "gsm8k_platinum",
                                      "gsm8k-plat", "platinum"]:
                    ds = load_dataset("madrylab/gsm8k-platinum", "main", split="test")
                    idxs = random.sample(range(len(ds)), min(len(ds), per_ds))
                    for i in idxs:
                        q = ds[i].get("question", "")
                        raw_a = ds[i].get("answer", "")
                        a = (raw_a.split("####")[-1].strip().replace(",", "")
                             if "####" in raw_a else raw_a)
                        item = {
                            "puzzle": self._maybe_harden(q, hardener),
                            "answer": a,
                            "dataset": "gsm8k-platinum",
                            "id": f"gsm8k-platinum_{i}",
                        }
                        pool.append(item)

                # ----------------------------------------------------------------
                # [NEW 2026] NVIDIA OpenMathInstruct-2 — problems whose solutions
                # were generated by Llama-3.1-405B-Instruct.
                # Keys: "openmathinstruct2" (gsm8k-derived, the default),
                #       "openmathinstruct2_math" (MATH-derived)
                # HuggingFace: nvidia/OpenMathInstruct-2 (split="train")
                # Fields: problem, expected_answer, generated_solution,
                #         problem_source ("augmented_gsm8k" / "augmented_math")
                #
                # READ BEFORE PUTTING THIS IN A TABLE. The gold here is not a
                # human label: it is the answer a large model converged on, kept
                # when its solutions agreed. That is the same class of gold this
                # project has just shown to be 17% defective in gsm-hard, so it
                # cannot be used to argue that a method is CORRECT -- only that
                # two methods differ, and only with the caveat stated. It is also
                # training data for open models, so contamination is possible and
                # a 7B math model may have seen these problems. 14M rows, so it
                # is streamed and deduplicated by problem text rather than
                # downloaded.
                # ----------------------------------------------------------------
                elif ds_name_norm in ["openmathinstruct2", "openmathinstruct",
                                      "omi2", "openmathinstruct2_math"]:
                    want_math = ds_name_norm.endswith("_math")
                    want_src = "augmented_math" if want_math else "augmented_gsm8k"
                    stream = load_dataset("nvidia/OpenMathInstruct-2", split="train",
                                          streaming=True)
                    stream = stream.shuffle(seed=random.randint(0, 2**31 - 1),
                                            buffer_size=10_000)
                    seen_q, taken, scanned = set(), 0, 0
                    for rec in stream:
                        scanned += 1
                        if taken >= per_ds or scanned > 200 * per_ds + 20_000:
                            break
                        if str(rec.get("problem_source", "")) != want_src:
                            continue
                        q = str(rec.get("problem", "")).strip()
                        a = str(rec.get("expected_answer", "")).strip()
                        if not q or q in seen_q:
                            continue
                        try:                       # numeric golds only: the
                            float(a.replace(",", ""))   # grader compares numbers
                        except ValueError:
                            continue
                        seen_q.add(q)
                        pool.append({
                            "puzzle": self._maybe_harden(q, hardener),
                            "answer": a.replace(",", ""),
                            "dataset": f"openmathinstruct2:{want_src}",
                            "id": f"openmathinstruct2_{taken}",
                        })
                        taken += 1

                # ----------------------------------------------------------------
                # [NEW 2026] NVIDIA OpenMathReasoning — AoPS competition problems,
                # reasoning traces generated by DeepSeek-R1 / Qwen (AIMO-2 winner).
                # Keys: "openmathreasoning", "omr", "nvidia_math"
                # HuggingFace: nvidia/OpenMathReasoning (split="cot")
                # Fields: problem, expected_answer, pass_rate_72b_tir,
                #         problem_source, problem_type, generation_model
                #
                # `pass_rate_72b_tir` is how often a 72B model with a tool loop
                # solved it, which is both a difficulty dial and a corroboration
                # signal: a rate of 0 means nothing independently reproduced the
                # stated answer. The default band keeps problems a 7B model can
                # plausibly reach while excluding uncorroborated golds. Same two
                # caveats as OpenMathInstruct-2: model-derived gold, and possible
                # contamination. Streamed, not downloaded.
                # ----------------------------------------------------------------
                elif ds_name_norm in ["openmathreasoning", "omr", "nvidia_math"]:
                    lo, hi = 0.25, 0.95          # corroborated, not trivial
                    stream = load_dataset("nvidia/OpenMathReasoning", split="cot",
                                          streaming=True)
                    stream = stream.shuffle(seed=random.randint(0, 2**31 - 1),
                                            buffer_size=10_000)
                    seen_q, taken, scanned = set(), 0, 0
...
Select the most reliable candidate."""
    ...
    def _solve_continue(self, problem: str, expected: str,
        ...
        siv_independent = None  # [v16.4] audit against an INDEPENDENT reference
        ...
        if (programmer_response.confidence > 0.5
            ...
            if self.enable_siv and SYMPY_AVAILABLE and answer_num is not None and blackboard_logic.get("equations"):
                ...
                # [v16.4] THE REFERENCE PROBLEM.
                #
                # The audit above compares the blueprint against
                # programmer_response.answer -- but the Programmer wrote its
                # code FROM blackboard_logic, the very blueprint being checked.
                # The reference is generated by the thing under test, so the
                # comparison is near-tautological: measured over four runs,
                # execution_rel_error is EXACTLY zero on 81% of rows, the audit
                # reports passed on 23 of the 27 errors, and `verified` is True
                # on 20 of them. That is why every selector built on this
                # telemetry failed out-of-sample -- the telemetry was vacuous
                # by construction, not merely noisy.
                #
                # Verification needs a reference that does NOT share an origin
                # with the artefact. The zero-shot baseline is exactly that,
                # and it is already computed on every problem, so a second
                # audit against it costs one symbolic evaluation and no LLM
                # call. Same code, same TOLERANCE_REL of 1e-4 -- only the
                # reference changes:
                #
                #   reference    rows  exactly 0   near-band   value
                #   Programmer    539        81%     2 rows    dead
                #   baseline      532        48%    20 rows    W=10 L=2, p=0.039
                #
                # Kept ALONGSIDE the coupled audit rather than replacing it, so
                # that the four finished runs stay comparable and both can be
                # read off the same run.
                _base_num = _extract_last_number(str(base_ans)) if base_ans is not None else None
                if _base_num is not None:
                    try:
                        siv_independent = SymbolicInverseVerifier.verify(
                            blackboard_logic, _base_num)
                        logger.info(
                            "[v16.4] independent audit (vs baseline): "
                            "passed=%s rel_error=%s  |  coupled (vs Programmer): "
                            "passed=%s rel_error=%s",
                            siv_independent.execution_audit_passed,
                            siv_independent.execution_rel_error,
                            siv_result.execution_audit_passed,
                            siv_result.execution_rel_error)
                    except Exception as exc:
                        logger.warning("independent SIV audit failed: %s", exc)
